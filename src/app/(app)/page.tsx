...
import { Bot, Calendar, ChevronRight, FileText, PackageCheck, Target, Wallet } from 'lucide-react';
    ...
    <div className="space-y-8">
          ...
          <Card className="rounded-3xl border border-slate-900/60 bg-slate-950/70">
            ...
            <CardContent className="space-y-3">
              {loading.events ? (
                ...
                events?.map((event) => (
                  <div key={event.id} className="flex items-center gap-3 rounded-2xl bg-slate-900/60 p-3">
                    <div className="rounded-xl bg-slate-800/80 p-2 text-xs text-slate-300">
                      <p>
                        {(event.start as any)?.toDate?.().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }).split(' ')[0]}
                      </p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="rounded-3xl border border-slate-900/60 bg-slate-950/70">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-lg font-semibold text-white">
              <PackageCheck className="h-5 w-5 text-cyan-300" /> Shopping assistant
            </CardTitle>
            <Button variant="ghost" size="sm" asChild>
        ...
        {loading.goals ? (
          ...
          goals?.map((goal) => (
            <GoalCard
              ...
              deadline={(goal.deadline as any)?.toDate?.().toLocaleDateString() ?? 'N/A'}
